...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine($"Максимальная плотность населения: {maxPlotnost:F2} чел/км²");
        Console.WriteLine($"Государство: {gosudarstva[indexMax]}");
        Console.WriteLine($"Население: {naselenie[indexMax]} млн");
        Console.WriteLine($"Площадь: {ploshad[indexMax]} тыс. км²");
    }
}